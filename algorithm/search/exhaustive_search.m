function exhaustive_search(f, interval, percentage_accuracy, is_min)
    ...
    % xf - fi / n <= percentage == n/(xf - fi) <= 1/(percentage) == n <= 1/(percentage)(xf - fi)
    initial_range = interval(2) - interval(1)
    n = 1 / (.01 * percentage_accuracy * initial_range)
    step = initial_range / n
    ...
    while (true)
        ...
        % Verifica se limite do intervalo foi alcancado
        if (i == n)
            break
        end
