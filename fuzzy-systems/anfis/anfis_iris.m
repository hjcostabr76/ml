...
%% Read dataset
ds = readtable('./file/ds_iris.csv');
% summary(ds);
nRows = size(ds, 1);

features = ds.Properties.VariableNames(2:end-1);
classCol = ds.Properties.VariableNames(end);
mainClass = 'Iris-setosa';
nFeatures = length(features);

X = ds{:, features};
Y = double(convertCharsToStrings(ds{:, classCol}) == mainClass);
